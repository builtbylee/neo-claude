#!/usr/bin/env python3
"""Run a synthetic analyst pilot with 3 Claude personas over a shadow cycle."""
...
def _ask_persona(
    ...
    for attempt in range(2):
        ...
        try:
            ...
            break
        except Exception as exc:  # pragma: no cover - network/LLM fallback
            last_error = exc
            continue
...
def _summarize(run_name: str, cycle: dict[str, Any], rows: list[dict[str, Any]]) -> dict[str, Any]:
    by_item: dict[str, list[dict[str, Any]]] = defaultdict(list)
    for r in rows:
        by_item[r["shadow_cycle_item_id"]].append(r)

    recommendation_counts = Counter(r["recommendation_class"] for r in rows)
    ...
    for item_id, decisions in by_item.items():
        ...
        model_recs = {
            d.get("model_recommendation")
            for d in decisions
            if d.get("model_recommendation")
        }
        if model_recs:
    ...
    return {
        ...
        "modelMajorityAlignmentRate": round((model_alignment_hits / model_alignment_checks), 4)
...
def _write_report(summary: dict[str, Any], output_dir: Path) -> tuple[Path, Path]:
    ...
    md = [
        f"# Synthetic Analyst Pilot — {summary['runName']}",
        "",
        f"- Cycle: `{summary['cycleName']}` ({summary['cycleId']})",
        f"- Deals evaluated: {summary['totalItems']}",
        f"- Decisions generated: {summary['totalDecisions']} (3 analysts per deal)",
    ...
    md.append("")
    md.append("## All Recommendation Distribution")
    md.append("")
...
def main(
    cycle_id: str | None = typer.Option(
        None,
        help="Shadow cycle id (defaults to latest active cycle).",
    ),
    max_items: int = typer.Option(25, help="Max shadow-cycle deals to evaluate."),
    ...
    if not settings.anthropic_api_key:
        raise typer.BadParameter("SL_ANTHROPIC_API_KEY is required for synthetic pilot.")
    ...
    try:
        ...
        name = run_name or f"synthetic_pilot_{_quarter_label(date.today())}_{cycle['cycle_name']}"
        ...
        run_row = execute_query(
            ...
            (
                ...
                MODEL,
                max_items,
                "Three synthetic analyst personas (conservative/balanced/aggressive)",
            ),
        )
        run_id = run_row[0]["id"]
        ...
        for item in items:
            ...
            for persona in PERSONAS:
                try:
                    result = _ask_persona(client, persona, context)
                except Exception as exc:  # pragma: no cover - runtime fallback
                    logger.warning(
                        "synthetic_pilot_persona_failed",
                ...
                execute_query(
                    ...
                    (
                        run_id,
                        item["shadow_cycle_item_id"],
                        persona["id"],
                        result["recommendation_class"],
                        result["conviction"],
                        result["rationale"],
                        json.dumps(result["key_risks"], default=_json_default),
                        json.dumps(result["data_gaps"], default=_json_default),
                        json.dumps(result["raw"], default=_json_default),
                    ),
                )
                decisions_for_summary.append(
                    {
        ...
        summary = _summarize(name, cycle, decisions_for_summary)
        ...
        logger.info(
            "synthetic_pilot_completed",
            run_id=run_id,
            run_name=name,
            items=len(items),
            decisions=len(decisions_for_summary),

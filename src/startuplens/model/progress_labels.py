...
WITH campaign_base AS (
    ...
    WHERE c.source = 'sec_dera_cf'
      ...
      AND co.campaign_date <= CURRENT_DATE - INTERVAL '18 months'
...
follow_on AS (
    ...
    JOIN sec_cf_filings scf
        ...
        AND scf.filing_date <= cb.campaign_date + INTERVAL '24 months'
...
revenue_progress AS (
    ...
    JOIN financial_data fd2
        ...
        AND fd2.period_end_date <= cb.campaign_date + INTERVAL '24 months'
...
def load_progress_labels(
    ...
    """
    rows = execute_query(conn, _PROGRESS_LABEL_QUERY, (start, end))
